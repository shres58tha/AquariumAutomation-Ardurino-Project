...
///*rotary encoder with button + 1 button input + 1 button feed interrupt*/
  ...
  enum Rot_Encoder_Event {NO_CHANGE, CLOCKWISE, ANTICLOCKWISE, SW_BUTTON};
  String Rot_Encoder_Event_String[5] {"NO_CHANGE", "CLOCKWISE", "ANTICLOCKWISE", "SW_BUTTON"};
...
uint16_t getLux(){  // returns Lux normal scale
  ...
  */
      ...
      //pinModeFast(currentPinLux ,INPUT);    //make analog pin A8 as input these prevent irratic jumps as present int he pH probe capacitative swings
      uint8_t n=3;
...
uint16_t pH=0;// cal_val = -2.0;
      
#ifdef __DEBUG__PH_ 
      uint8_t n=4;  

      for(int i=0;i<n;i++){
      pH += analogRead(currentPin_pH);
      delay(10);
      }
      //pH = -5.70 *pH*(5.0/1024)/n + 21.34 + cal_val;
      pH = -11*(pH/10) - pH/100 + 193;  //optimsed using int math same as above formula with decimal correct to 1 place 
      Serial.print( F("pH:") );
      Serial.println(pH);// delay(2000);
...
union{
	struct{
		byte blank=0;																	//1 byte  1
		byte flag=0;																	//1 byte  2    valid data means flag 1
		uint16_t setTemp;														  //2 byte  4    in temp*10 scale
		uint16_t setLux;															//2 byte  6    int lux*10 scale
		//uint16_t  setLightOnDuration;							    //4 byte  10    millis set light on duration for option to turn off at night
    uint16_t feedTime1=540;                       //2 byte  12
    uint16_t feedTime2=1080;                      //2 byte  14    in minutes  hh*60+mm
	};																							//14 bytes
	byte byteData[14];															//for saving these into EEPROM // 2 forextra save space
} x ;
...
//variable working here
  ...
  uint8_t range_pH= 10;                             //+-10 ie ph 6 to 8  deci scale
//
...
//output pin group
  ...
  #define  controlPinFilter 33									// default state low high cutoff
...
String MenuLvl1_message[NumMenu]= {      // make sure the pointer  i has a limit set . currently all ram can be viewed by rotating below exit 
  //    0123456789012
       "Set Temp",  //0
       "Set lux",  //1
       "Set Clock",  //2
