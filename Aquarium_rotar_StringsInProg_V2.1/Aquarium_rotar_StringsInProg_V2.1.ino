...
union {
  struct {
    bool blank = 0;             //1 bool  1
    bool flag = 0;              //1 bool  2    valid data means flag 1
    ...
    uint16_t feedTime2 = 1080;  //2 byte  14    in minutes  hh*60+mm            //overwritten by one eeprom
  };                            //14 bytes
...
#define controlPinFoodServo 38  //pwn may be needed servo motor turn pin. Normal low. introduce feed when high
#define controlPinVibrator 39
...
void servoSweep() {  //may need debugging
  ...
  if (state.FoodServe) {  //HIGH then process below
    
    digitalWriteFast(controlPinVibrator,state.FoodServe); //vibrator on if state.FoodServe is on
    

    #ifdef __DEBUG__servo_
        Serial.println(F("state.FoodServe:1"));
        Serial.println(F("controlPinVibrator:1"));
    ...
    if (millisCurrent - millisLastServo >= servoInterval) {
      ...
      digitalToggleFast(controlPinVibrator);
    ...
    if (servoDegrees <= servoMinDegrees) {  // servo returned to initial position
      state.FoodServe = LOW;
      ...
      digitalWriteFast(controlPinVibrator,state.FoodServe); //vibrator off if state.FoodServe is on

      #ifdef __DEBUG__servo_

            Serial.print(F("state.FoodServe: "));Serial.println(state.FoodServe);
            Serial.print(F("controlPinVibrator: "));Serial.println(state.FoodServe);
...
void ControlState() {  //need debug here check the outputs
  ...
  uint16_t checkFeedTime = rtc_now.hour() * 60 + rtc_now.minute();
  ...
  if (abs(current_pH - normal_pH_7) > 15 ){
  ...
  //food control
    ...
    Serial.print(F("state.FoodServe ->"));   
    Serial.Println(state.FoodServeCaller);
    Serial.print(F("state.FoodServe1 ->"));   
    Serial.Println(state.FoodServeCaller);
  #endif


  if (state.FoodServe1 == LOW) {  //pin low
    if ( (checkFeedTime != 0) && (checkFeedTime - x.feedTime1) > 0) {
      state.FoodServeCaller = HIGH;      //becomes high servo on 
  ...
  if (state.FoodServe2 == LOW) {  //pin low
    if ((checkFeedTime != 0) && (checkFeedTime - x.feedTime2) > 0) {
      state.FoodServeCaller = HIGH;      //pin high food served
      state.FilterCtrlPin = HIGH;  //pin high here filter off
  ...
  if (state.FoodServeCaller == HIGH) {
    digitalWriteFast(controlPinFilter,state.FilterCtrlPin);  // set filter pin high ie turn off the filter
    servoSweepCaller();
    if (state.FoodServeCaller == LOW) {  // servo has finished its service
...
//set out pin all digital 
  ...
  pinModeFast(controlPinVibrator,OUTPUT);
...
//set initial environment
  ...
  digitalWriteFast(controlPinVibrator,LOW);
