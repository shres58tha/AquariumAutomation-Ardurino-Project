// STRIPPED FILE 
#define default_val \
  { 0x0, 0x1, 0xE6, 0x0, 0xC2, 0x1, 0x1C, 0x2, 0x38, 0x4 }
//for the x

// #define __SERIAL_BEGIN_
// #define __DEBUG_LCD_DEMO_
// #define __INTIAL_TIME_ESTIMATION_0
...
uint16_t getCelsius(DeviceAddress deviceAddress) {  //working  dont touch
  ...
  uint16_t tempC = 10 * oneWireSensors.getTempCByIndex(0);  //first tempSensor is .5 degree accurate
  return tempC ;
  // return 10 * oneWireSensors.getTempCByIndex(0);
}
...
uint8_t getPH() {  //returns in pH*10    //returns deci scale  //running average  //non blocking and avioids sudden jumps
    ...
    if (i < size) {
      ...
      for (int j = 0; j < size - 1; j++) {
        pHs[j] = pHs[j + 1];  //pHs[size = last-1] is freed
      }
      pHs[size - 1] = val;
      sum += val;
    }
...
struct {
  bool FoodServe = LOW;
  bool FoodServing = LOW;  //reset these at  control at hhmm 0000 or hhmm=0;
  bool FilterControl = LOW;  // normal state is low --> filter pump on
  bool servoDirection = HIGH;
...
union {
  struct {
    struct{
      bool flag;                    //1 byte      valid data if flag%2 == 1
      bool SoundAlarm;
    };
  ...
  byte byteData[10] = default_val;  //default values
} x;
...
Rot_Encoder_Event Rot_Encoder_Poll() {
  ...
  while ((millis() - millisInitial) < millisTimeout) {
    ...
    if (state.SW_Last != state.SW_Current) {  // detecting falling edge
      ...
      if (!state.SW_Current) {
        delay(15);                //some bouncing effect seen for debounce
        return SW_BUTTON;
      }
    }
  }
...
void Blink(uint8_t intensity = 200, uint16_t blinkDuration = interval) {  //default is half power led intensity offa t below 150
  static bool state;
...
void ControlState() {    //nonblocking
  
  static uint8_t servoPosition = servoMinDegrees;
  static uint16_t filterOffEvent;
  uint8_t range_pH = 15;     // used deci
  uint8_t normal_pH_7 = 70;  // used deci

  // if (currentTemp < x.setTemp - 5) {          // turn on if less than .5 degree lower ie 1 deg lowerthan set
  ...
  if (currentTemp < x.setTemp ) {           
    digitalWriteFast(controlPinHeater, LOW);   // becomes LOW  heater on
  } else if (currentTemp > x.setTemp )  {      // 1 degree hysterisis         //nope tis assumption is wrong hysterisis loop auto included as thermometer resolution is .5
    digitalWriteFast(controlPinHeater, HIGH);  //becomes HIGH   heater off
  }
  ...
  else if (currentLight > x.setLux + 100) {
  ...
  if( (current_pH > normal_pH_7 + range_pH) || (current_pH < normal_pH_7 - range_pH) ){  //stored units are decis ie 20.0 stored as 200
    alarm(HIGH);
  } else if (( x.setTemp > currentTemp + 20) || (currentTemp > x.setTemp + 20) ){
      alarm(HIGH);    
    }
  // else if (currentTemp <= clockTemp) {
  //   alarm(HIGH);
  // }
  else {
    alarm(LOW);
  ...
  if (checkFeedTime == x.feedTime1)  {
    if (state.FoodServing == LOW) {  //pin low
      state.FoodServe = HIGH;        //becomes high servo on
      state.FoodServing = HIGH;
    }

  } 
  else if (checkFeedTime == x.feedTime2) {
    if (state.FoodServing == LOW) {
      state.FoodServe = HIGH;  //pin high food served
      state.FoodServing = HIGH;
    }

  } else {
    state.FoodServing = LOW;
  }

  // these code should yield low size but is yielding higher ??
      
  // state.FoodServe = ((checkFeedTime == x.feedTime1) | (checkFeedTime == x.feedTime2)) & ~(state.FoodServing);
  // state.FoodServing = state.FoodServe;

  // if (state.FoodServing == LOW){
  //   if ((checkFeedTime == x.feedTime1) || (checkFeedTime == x.feedTime2)) {
  //       state.FoodServe = HIGH;  //pin high food served
  //       state.FoodServing = HIGH;
  //   }
  //   else {
  ...
  if (state.FoodServe == HIGH) {  //wrapping it in funtion does not work why??
    state.FilterControl = HIGH;   //means filter off
    ...
    if (servoRuns < servoRunTime) {
      if (servoPosition >= servoMinDegrees) {  
        ...
        if (servoPosition >= servoMaxDegrees) {
          state.servoDirection = LOW;
        }

        if (servoPosition <= servoMinDegrees) {
          state.servoDirection = HIGH;
          servoRuns++;
        }
  ...
  if (state.FilterControl) {  //ie filter is off						// if filter pin is high toggle it on after delay of 30 min
    if (checkFeedTime - filterOffEvent >= 30) {
      state.FilterControl = LOW;  // low means filter pump on countdown to start filter pump started
    }
  }
  digitalWriteFast(controlPinFilter, state.FilterControl);
