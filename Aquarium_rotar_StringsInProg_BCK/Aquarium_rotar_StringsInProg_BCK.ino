...
//Debug 
  ...
  //for working sound alarm  #define _SoundALarm_ON
  ...
  */
  ...
  #define backlightPin 11  //pwm capable pin
      ...
      void lcdLnClr(uint8_t lineNo);          //declaration of lcdLnClr() small function to clear line
...
///*rotary encoder with button + 1 button input + 1 button feed interrupt*/
  ...
  String Rot_Encoder_Event_String[5] {"NO_CHANGE", "CLOCKWISE", "ANTICLOCKWISE", "SW_BUTTON", "TIME_OUT"};
...
//input pin group
  /*  if used potentiometer for input 
  #define  setPinTemp  A0														//pin to read from pot1         uint8_t = byte
  #define  setPinLux  A1														//in A4*conversionFactor/1024   pin to read from pot2
  #define  setPinFeedingInterval A2									//pin to read from pot3
  #define  setPinLightOnDuration A3									//pin to read from pot4
  #define  setPinReqPushButton 22										//push and hold this pin to set parameters by rotaing pots pull to low using registor
*/
...
uint16_t getCelsius(DeviceAddress deviceAddress){ //working  dont touch
  ...
  float tempC = oneWireSensors.getTempCByIndex(0);  //first tempSensor
  

  #ifdef __DEBUG__TEMP_
      //Serial.println(' ');
      Serial.print( F("\nTemperature read from the thermometer") );Serial.println(tempC,1);
  #endif

  return(uint16_t) (tempC*10.0);
}
...
uint16_t getLux(){  // returns Lux normal scale
  ...
  */
  ...
  pinMode(currentPinLux ,INPUT);    //make analog pin A8 as input these prevent irratic jumps as present int he pH probe capacitative swings
  uint8_t n=3;
...
uint8_t getPH(){  //returns in pH*10    //needs debugging here too
  float V0=0.0,pH, cal_val = -2.0;
  uint8_t n=3;  

  for(int i=0;i<n;i++){
  V0 += analogRead(currentPin_pH);
  delay(10);
  }
...
//Servo  constants
 ...
 int servoDegrees = 5;		
 												//amount servo moves at each step
// Structure state flags
...
union{
	struct{
		...
		uint16_t setLux;															//1 byte  3    int lux*10 scale
		unsigned long setLightOnDuration;							//4 byte  7   millis set light on duration for option to turn off at night
    uint16_t feedTime1=540;                       //4 byte  11
    uint16_t feedTime2=1080;                      //4 byte  15    in minutes  hh*60+mm
	};																							//16 bytes
	byte byteData[20];															//for saving these into EEPROM // 2 forextra save space
} x ;

//variable working here
  ...
  const uint8_t range_pH= 10;                             //+-10 ie ph 6 to 8
//
...
//
void alarm( uint8_t state) {  //works properly
  //state=LOW;


  #ifdef __DEBUG__CTRL_
      Serial.print(  "alarmState:"  )
      Serial.print(ln  state  );  //entering here
  #endif
...
void servoSweep() {  //not debuged yet
  

  #ifdef __DEBUG__servo_
      Serial.println(  F("INSIDE ServoSweep") );
  #endif
  ...
  #ifdef __DEBUG__servo_
      Serial.println(  F("state.FoodServe:1") );
  #endif
 ...
 //make the servo move to the next position
       ...
       Serial.println(servoPosition)l
  #endif
 ...
 //and record the time when the move happened
  ...
  #ifdef __DEBUG__servo_
      Serial.println(  F("state.FoodServe:0\n FOOD SERVED") );
  #endif      
...
void ControlState() {   //need debug here check the outputs
  ...
  uint16_t feedtime = rtc_now.hour()*60 + rtc_now.minute();
  ...
  if (feedtime = 0){  // midnight reset the feed flaga
  ...
  #ifdef __DEBUG__CTRL_
      Serial.print(  F("pH °C->") );  //entering here
  #endif
  ...
  //food control 
	...
	if ( state.FoodServe == LOW && state.FoodServe1 == LOW) {									//pin low
		if ( ( feedtime - x.feedTime1) > 0  ) {  
			state.FoodServe =  HIGH;										//becomes high
			millisLastFed = millisCurrent + millisFoodInterval;
			state.FilterCtrlPin= HIGH;                  //pin high here filter off

      

  #ifdef __DEBUG__CTRL_
          Serial.print(  F('1') );  //entering here
  #endif

		}
	}

	if ( state.FoodServe == LOW && state.FoodServe2 == LOW) {									//pin low
		if ( ( feedtime - x.feedTime2) > 0  ) {  
			state.FoodServe =  HIGH;										//becomes high
			millisLastFed = millisCurrent + millisFoodInterval;
			state.FilterCtrlPin= HIGH;                  //pin high here filter off
  ...
  #ifdef __DEBUG__CTRL_
          Serial.print(  F('2') );  //entering here
  #endif
	...
	if ( state.FoodServe == HIGH ) {
		...
		if (state.FoodServe == LOW) {									// servo has finished its service
			...
			state.FilterCtrlPin= LOW;								// low means filter pump on
		}
	}
  ...
  //turn filter pump after millisFilterPumpOffTime
	...
	if ( !state.FilterCtrlPin )									// if filter pin is low turn it on after delay of 30 min in millisFoodInterval
		if ( millisCurrent - millisFilterOffCommence >= millisFoodInterval)
  //start the filter pump again
			digitalWriteFast(controlPinFilterPump, state.FilterCtrlPin) ;


  #ifdef __DEBUG__CTRL_
      Serial.println(  state.FilterCtrlPin );  //entering here
  #endif
...
//RCT clock   //pin 18 rx and 19 tx
  ...
  #ifdef __INTIAL_TIME_ESTIMATION_
      Serial.println (  millisCurrent - millis() );
  #endif
...
//Rotory encoder and button interrupts
  ...
  pinModeFast(Feed_Interrupt_Button, INPUT_PULLUP);
...
void lcdLnClr(uint8_t lineNo){
  ...
  }  //lineNo<2?lcd.setCursor (0,lineNo):lcd.setCursor (-4,lineNo);
        //     0123456789012345
  lcd.print(F("                ")) ;
  //lcd.print("                ")) ;  
...
void lcdmenu(lcd_option option){                 //stable works do not tweak further
  ...
  switch (option) {
    ...
    case setupEEPROM:  // this should be blocking 
      lcd.clear();
   //                                     0123456789012345
      lcd.setCursor (0,0);  lcd.print( F("EEPROM Data ERR") );
      lcd.setCursor (0,1);  lcd.print( F("DO INTIAL SETUP") );
      //lcdLnClr(2);
      //lcdLnClr(3);
      delay(1000);
      inputParameters();
    ...
    case Update_EEPROM:
      lcd.clear();
    //                                 0123456789012345
      lcd.setCursor (0,0);  lcd.print( F("settingUp") );
      //lcd.setCursor (0,1);  lcd.print('0);
      lcd.setCursor (-4,2); lcd.print( F("input please") );
    ...
    case LCD_Display:     // update tweak for layout formating when all is finished
      ...
      byte time_hh=rtc_now.hour();
      byte time_min=rtc_now.minute();
...
String MenuLvl1_message[NumMenu+1]= {      // make sure the pointer  i has a limit set . currently all ram can be viewed by rotating below exit 
  //    0123456789012
       "Set Temp.    ",  //0
       "Set lux      ",  //1
       "Set Clock    ",  //2
       ...
       "Set FeedTime2",  //4
       "Exit"         ,  //5
       "MemProtector"    //6  // note to self actually not needed  but will change the code later
};
...
bool f_setTemp(){
  ...
  while(1){                    // infinite loop blocking
                               //    0123456789012345
      lcdLnClr(1);lcd.setCursor(0,1); lcd.print( F("Temp Setting") );
      lcdLnClr(2);lcd.setCursor(0,2); lcd.print(x.setTemp/10.0) ;lcd.write(1);     //lcd line 2 and 3 puts tabs at intial so must be select -ve column
...
bool f_setLux(){
  ...
  while(1){                    // infinite loop blocking
                               //    0123456789012345
      lcdLnClr(1);lcd.setCursor(0,1); lcd.print( F("Light Setting") );
      lcdLnClr(2);lcd.setCursor(0,2); lcd.print(x.setLux);
...
bool f_setTime(byte sel){
  ...
  while(1){  
    ...
    lcdLnClr(0);lcd.setCursor(5,0); lcd.print(str[sel]);
    while(selector==0){
          ...
          Serial.println(  F("in the fn f_setTime while selector == 0 loop") ) ; 
                         ...
                         //                0123456789012345
      lcdLnClr(1);lcd.setCursor(0,1); lcd.print( F("set HH (24)") );
      lcdLnClr(2);lcd.setCursor(0,2); //lcd.print(hh) ;
  ...
  #ifdef __DEBUG__f_setTime_
        Serial.print(  F("before while selector == 1 loop  :") ); Serial.println( selector) ;
  #endif
    ...
    while(selector==1){
          ...
          Serial.print(  F("in while selector == 1 loop  :") ); Serial.println( selector) ;
  #endif

                               //   0123456789012345
      lcdLnClr(1); lcd.setCursor(0,1); lcd.print( F("set mm") );
      lcdLnClr(2); lcd.setCursor(0,2);// lcd.print(mm) ;
  ...
  #ifdef __DEBUG__f_setTime_
            Serial.println( F("code to update clock / starttime /interval") );
  #endif
              ...
              switch(sel){
                case 0:
  ...
  #ifdef __DEBUG__f_setTime_
                      Serial.println( F("now.setTime(hh, mm, 0);   setting the time") );
  #endif
                  ...
                  rtc.adjust(DateTime(rtc_now.year(), rtc_now.month(), rtc_now.day(), hh, mm, 30 ));  //working now
...
void inputParameters(){
  ...
  while(whileFlag){
    ...
    for(uint8_t i = menuTop,j=0; i<menuTop+4; i++,j++) {   //mem smashing occuring here
          ...
          Serial.println( F("\n\n") );   // this is the culprit bug
  #endif

      if ( j<2) {      // these brackets are must these if not in one line code cause error why??
        lcd.setCursor(2,j);
        }
      else {
        lcd.setCursor(-2,j);
    ...
    switch ( Rot_Encoder_Poll() ) {     // unprermitted memory access problem posssibly interplay of menuposition and menutop
        ...
        case CLOCKWISE:
          ...
          if (menuPosition >=4 && menuPosition < NumMenu) //scrolls
            menuTop++;    
          if( (menuTop+4) > (NumMenu) ) //this fix the issue but need to understand why  here 4 is the no of line in the lcd
            menuTop--;
