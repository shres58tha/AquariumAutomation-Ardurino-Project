...
void ControlState() {  //need debug here check the outputs  //nonblocking
  ...
  if (state.FoodServe == HIGH) {  //wrapping it in funtion does not work why??
    ...
    if (feedserved < feedtimes) { 
      ...
      uint8_t servoPosition = servoMinDegrees;
      while (servoPosition >= servoMinDegrees) {  //blocking taking 800ms  how to make it unblocking??
        ...
        while( servoPosition != servoFeed.read()){
          servoFeed.write(servoPosition);
        }
      }*/
      while(servoFeed.read() <= servoMaxDegrees)
          servoFeed.write(servoMaxDegrees);
      while(servoFeed.read() >= servoMinDegrees)
